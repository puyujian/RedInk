...
所有重要数据都通过 Docker volumes 持久化存储：
...
```env
# API 配置
GOOGLE_CLOUD_API_KEY=         # Gemini API Key（必填）
IMAGE_API_KEY=                # 图片生成 API Key（必填）
...
# JWT 配置
JWT_SECRET_KEY=               # JWT 密钥（必填，生产环境必须修改）
